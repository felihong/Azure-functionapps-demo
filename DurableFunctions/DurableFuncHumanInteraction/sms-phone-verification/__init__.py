...
def orchestrator_function(context: df.DurableOrchestrationContext):
    ...
    challenge_code = yield context.call_activity("send-sms-challenge", phone_number)
    expiration = context.current_utc_datetime + timedelta(minutes=3)
    timeout_task = context.create_timer(expiration)

    authorized = False
    for _ in range(3):
        challenge_response_task = context.wait_for_external_event("sms-code")

        # Decide if escalate (timeout) or proceed with authorization
